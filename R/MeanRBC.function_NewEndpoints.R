...
meanRBC.function_NewEndpoints <- function(time, APE) {
  ...
  ### Extra step for this data: 
  # sub1$end_point[nrow(sub1)] <- sub1$time[nrow(sub1)]
  ...
  if(sum(sub1$end_point) == 0) {
    ...
    sub1[nrow(sub1) + 1,] <- NA # add a blank row 
    sub1$end_point[nrow(sub1)] <- new_end_time
  }
  ...
  # make an if statement: make sure this always works -> can this column be NA for any other reason? 
